...
import { initialFormasDePago } from '../../lib/data';
import { FormaDePago } from '../../lib/types';
import { EditIcon, DeleteIcon, PauseIcon, PlusIcon, SaveIcon, CancelIcon } from '../icons/Icons';

const FormasDePagoPage: React.FC = () => {
  const [formasDePago, setFormasDePago] = useState<FormaDePago[]>(initialFormasDePago);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editFormData, setEditFormData] = useState<FormaDePago | null>(null);

  const handleEditClick = (item: FormaDePago) => {
  ...
  const handleSaveClick = (id: number) => {
    ...
    const newFormasDePago = formasDePago.map((item) => (item.id === id ? editFormData : item));
    setFormasDePago(newFormasDePago);
    setEditingId(null);
    setEditFormData(null);
  };
  ...
  const renderReadOnlyRow = (item: FormaDePago) => (
    <tr key={item.id} className="hover:bg-gray-50">
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{item.nombre}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-4">
        <button onClick={() => handleEditClick(item)} className="text-indigo-600 hover:text-indigo-900" title="Editar"><EditIcon /></button>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-gray-700 text-3xl font-medium">Formas de Pago</h3>
        <button
        ...
        >
          <PlusIcon />
          <span className="ml-2">Agregar Forma de Pago</span>
        </button>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {formasDePago.map(item => (
              <Fragment key={item.id}>
                {editingId === item.id ? renderEditableRow() : renderReadOnlyRow(item)}
              </Fragment>
            ))}
          </tbody>
        </table>
